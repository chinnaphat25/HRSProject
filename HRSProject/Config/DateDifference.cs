...
namespace HRSProject.Config
{
    public class DateDifference
    {
        ...
        public string ToStringNew()
        {
            string retStr = string.Empty;
            if (this.Year > 0)
                retStr = retStr + string.Format("{0} {1} ", this.Year.ToString("#,##0"), " ปี ");
            if (this.Month > 0)
                retStr = retStr + string.Format("{0} {1} ", this.Month.ToString("#,##0"), " เดือน ");
            if (this.Day > 0)
                retStr = retStr + string.Format("{0} {1} ", (Day-1).ToString("#,##0"), " วัน ");
            return retStr.Trim();
        }
    }
}